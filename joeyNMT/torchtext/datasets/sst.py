...
from ..data.field import Field, LabelField
from ..data.iterator import BucketIterator
...
class SST(Dataset):
    ...
    def __init__(self, path, text_field, label_field, subtrees=False,
        ...
        Field.preprocessing = Pipeline(get_label_str) #:TODO 여기 틀릴수도 있음음
        with open(os.path.expanduser(path)) as f:
            if subtrees:
                examples = [ex for line in f for ex in
                            Example.fromtree(line, fields, True)]
            else:
