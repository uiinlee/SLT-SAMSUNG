...
# from torchaudio.models.decoder import CTCBeamDecoder
from pyctcdecode import build_ctcdecoder #  pip install https://github.com/kpu/kenlm/archive/master.zip 도 같이 함함

from joeyNMT.initialization import initialize_model
from joeyNMT.embeddings import (
    Embeddings,
    SpatialEmbeddings,
)
...
from joeyNMT.vocabulary import (
    TextVocabulary,
    GlossVocabulary,
    PAD_TOKEN,
    EOS_TOKEN,
...
class SignModel(nn.Module):
    ...
    def run_batch(
        ...
        if self.do_recognition:
            ...
            # gloss_scores: (N, T, C)
            gloss_scores = self.gloss_output_layer(encoder_output)
            # gloss_probabilities: (N, T, C)
            gloss_probabilities = gloss_scores.log_softmax(2)
            # pyctcdecode는 각 샘플마다 (T, C)를 입력받으므로,
            # 여기서는 배치 차원을 유지한 채로 CPU로 옮기고 numpy array로 변환합니다.
            emissions = gloss_probabilities.cpu().detach().numpy()  # shape: (N, T, C)

            # torchaudio의 CTC beam search 디코더 구성
            # decoder = _ctc_decoder(
            #     lexicon=None,                # lexicon-free 모드
            #     tokens=self.gls_vocab.itos,           # 반드시 사전에 blank token이 tokens[0]에 위치하도록 정의
            #     beam_size=recognition_beam_size,
            #     nbest=1                     # top_paths=1과 동일
            # )
            decoder = build_ctcdecoder(
                labels=self.gls_vocab.itos,
                kenlm_model_path=None,  # kenlm 없이 사용
                alpha=0.0,              # language model weight (필요 시 조정)
                beta=0.0                # word insertion penalty (필요 시 조정)
            )

            decoded_gloss_sequences = []
            for i, sample_emission in enumerate(emissions):
                # sample_emission: (T, C)
                decoded = decoder.decode(sample_emission)
                decoded_gloss_sequences.append(decoded)
                # logger.info(f"Sample {i} (Sequence: {batch.sequence[i]}) decoded output: {decoded}")
        else:
