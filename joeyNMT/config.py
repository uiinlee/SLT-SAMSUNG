...
BaseConfig = NamedTuple(
    ...
    [
        ...
        ("autocast", Dict),
        ("seed", int),
        ("training", TrainConfig),   # "train"에서 "training"으로 변경
        ("testing", TestConfig),     # "test"에서 "testing"으로 변경
...
def parse_global_args(cfg: Dict, rank: int, mode: str = "train") -> Dict:
    ...
    if fp16:
        autocast["dtype"] = torch.float16  # 필요시 torch.bfloat16으로 변경 가능
    ...
    args = {
        ...
        "training": parse_train_args(cfg["training"], mode),  # 'train'에서 'training'으로 변경
        "testing": parse_test_args(cfg["testing"], mode),    # 'test'에서 'testing'으로 변경
...
def parse_train_args(cfg: Dict = None, mode: str = "train") -> TrainConfig:
    ...
    recognition_loss_weight = cfg.get("recognition_loss_weight", 1.0)
    translation_loss_weight = cfg.get("translation_loss_weight", 1.0)
